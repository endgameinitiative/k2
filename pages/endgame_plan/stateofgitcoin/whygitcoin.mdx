...
<Callout type="info">
You can listen to some of our research [here](https://www.youtube.com/watch?v=PlDOjDu-mpU&list=PLF5CNzrKS9sqRxKaMqlh1tjWMCIBLBGkI).  Or you can play with Passport [here](http://passport.gitcoin.co/)
</Callout>

Basically the opportunity is to do for sybil resistence what twilio did for SMS or SendGrid did for SMTP.

Because Gitcoin Grants has always needed sybil resistence in order to function, Gitcoin has put a lot of work into the sybil resistence ecosystem, by way of integrating with top providers like: Idena, BrightID, Proof Of Humanity, etc.  But Gitcoin has never wanted to rely on one sybil resistence provider, so it has built a tool based on a plurality of providers.  There are dozens of "stamps" integrated into Gitcoin Passport, enabling sybil resistence to come from many places.  The end result is that Passport is like Kirby from Super Smash Bros: it can swallow the powers of everyone else.

The billion $$$ opportunity on Passport looks like this:

1. Bootstrap Gitcoin Passport with Gitcoin Grants.
1. The TAM for identity mechanisms is everyone on planet earth.
...
2. Our digital identities should not be monopolized by a single provider and/or people will want to benefit from a plurality of providers.
3. Integrate with Gitcoin Passport & get sybil resistence in a couple lines of code.

Along the way, it will be important to 
1. preserve privacy
